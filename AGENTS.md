...
- Schematics: `GET/POST /api/schematics`, `GET/PUT/PATCH/DELETE /api/schematics/{id}`, `GET /api/schematics/{id}/file` (alias `/download`), `GET/PUT /api/schematics/{id}/preview` (public GET returns the webp card thumbnail, 404 when absent; PUT is admin-only and caches a client-rendered webp, max 2 MiB). Metadata includes `hasPreview`; the client-rendered webp can be attached to uploads as the optional `preview` multipart field, and admins auto-backfill previews for older schematics when a card renders one client-side. List filters: `q,name,description,owner=me,sort,order,from,to,limit,offset,page,pageSize` (page/pageSize are 1-based aliases; response includes `total,totalPages`). `sort` accepts `uploadDate|updatedDate|name|size|rating|likes`. The frontend infinite-scrolls filtered pages instead of paginating client-side.
